    ...
    public function data()
    {
        ...
        return Datatables::of($article)
            ->addColumn('actions',function ($row){
               $url = '<a class="btn btn-success btn-sm" href="'.URL::to('admin/article/'.$row->id.'/edit').'">
                            <span class="glyphicon glyphicon-pencil"></span>
                            '. trans("admin/modal.edit") .'
                       </a>';
               $url.= '<a class="btn btn-danger btn-sm" href="'.URL::to('admin/article/'.$row->id.'/delete').'">
                            <span class="glyphicon glyphicon-trash"></span>
                            '.trans("admin/modal.delete") .'
                       </a> <input type="hidden" name="row" value="'.$row->id.'" id="row">';
              return $url;
            })->rawColumns(['actions'])
